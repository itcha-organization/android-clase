# 中間報告アウトライン

おはようごさいます！
おっと、つい日本語で、Buenos dias を言ってしまいました。
改めまして、スペイン語でおはようございます。
大沼響と申します。Chalatenangoで働いているコンピュータ技術のボランティアです。
本日はお越しいただき誠にありがとうございます。
プレゼンを始める前に1点お知らせです。検査のため、先ほど病院で血を提出してきました。ですので、もしプレゼンが終わる前に気絶したら、、、ごめんなさい。

それでは始めます。

## 自己紹介（簡単な経歴含む）
「好きこそものの上手なれ」"Querer es poder"
これは、私の座右の銘です。

自己紹介として私の好きなものを紹介します。

まず、私はITが好きです。

前職は、システムエンジニアです。
大学学部卒業後、3年間のWebアプリの開発と保守に携わりました。
プログラミング言語は、Java、JavaScript、Pythonを使っていました。
フレームワークは、GWT、SpringBoot、React.js、Flaskを使っていました。
バージョン管理には、Git、GitLabを使っていました。
IDEは、eclipse、InteliJ、VScodeを使っていました。
Kubernetes、ArgoCDの管理も少しだけ担当しました。
サイバーセキュリティとクラウドコンピューティングにはとても興味があり勉強中です。

このように、ITの世界は学ぶことがたくさんあって、私にとってとても面白いです。

ここで皆さんに1つ質問です。
最高で何枚のププサを一度に食べたことがありますか？
私は研修のイントロで毎回この質問をします。
5,6,7,8,10,11,12,13,14,それ以上！
何枚ですか？「」
Esooooooo!

ちなみに私は19枚です。
私が住んでいるチャラテナンゴ市のププサ大食い大会で4位になったときの写真です。
エルサルバドルに来てから、一番苦しかった瞬間はこの時でした。

## 配属先概要と要請内容紹介
### 任地
任地はChalatenango県Chalatenango市です。

### 配属先概要
私はITCHA（Instituto Tecnológico de Chalatenango）で働いています。
ITCHAは2年制のINSTITUTOS TECNOLÓGICOSです。
～ITCHAを知ってもらうためこちらの動画をご覧ください～
https://www.youtube.com/watch?v=QfUGp2OvVwQ
2025年度の入学受付中です。このチャンスをお見逃しなく。
ビデオの通りで、6つのCARRERAS TÉCNICASがあり、私はコンピュータ技術学科に所属しています。
コンピュータ技術には、教員3名、学生が約100人在籍しています。
2つの部分と活動しております。
1つ目は、私はコンピュータ技術学科の職員、学生向けの支援です。
2つ目は、情報系職員によるポータルサイト改善プロジェクトの支援です。

### 要請内容
私のミッションは次の3つです。
職員と生徒のAndroidアプリ開発知見を向上させる。
職員と生徒に新技術を紹介する。
学生の生産性を向上させる。

## 活動計画と1年目の活動実績
### 計画
そしてこちらの計画に沿って活動しております。
初年度はAndroid開発の知見向上とGitの知見向上に力を入れました。
本日は、主に1年目の実績と成果を紹介します。

### 活動1:Androidアプリ開発が知見が向上

プログラミング言語KotlinとフレームワークJetpackComposeを使ったAndroidアプリ開発の研修を実施しました。
（JetpackComposeは2021年にリリースされたモダンなフレームワークで需要が高まっています。）
プログラミング言語KotlinとフレームワークFlutterを使ったアプリ開発のイントロダクションを実施しました。
（Flutterは○○です。）
私は実施した活動を発表します。

#### 活動1-1:教員向け研修
Androidアプリ開発の教員向けの研修を実施しています。
テーマは、プログラミング言語KotlinとフレームワークJetpackComposeです。
現在はまでで3時間のセッションをn回実施しました。

#### 活動1-2:学生向け授業
カウンターパートと協力して、「Androidアプリ開発」モジュールの授業しました。
2つのグループに対して、全30コマ中に20コマの授業・教材作成を担当した。

内容は、プログラミング言語KotlinとJetpackComposeフレームワークを使ったAndroidアプリ開発に加えて、Flutter入門を実施しました。

実際に授業内で作成したアプリの例（CRUD、ゲーム、AIチャット）です。

#### 活動1-X:UMOAR特別授業
余暇時間を使ってITCHA外でも実施した活動です。
チャラテナンゴ県の私立大学UMOARで特別授業をさせて頂きました。

テーマはFlutter入門です。
3時間1回きりで、モバイルアプリ開発に必要なソフトウェアのインストールに割く時間がないことが課題でした。
なので、Webブラウザから利用可能な統合開発環境IDXを利用して、時間を節約しました。

### 活動2:新技術の紹介(Git)
ミッション達成のために、GitHubの研修を実施しました。
GitHubは、ソースコードを共有するためのツールで、チーム開発を効率化します。

また、教育機関関係者は、AIコーディングアシスタントサービスGitHubCopilotを無料で利用できるため紹介しました。

#### 活動2-1:職員向けGitHub研修
職員向けに3時間の研修を7セッション実施しました。
冒頭で話した通り、ポータルサイト改善プロジェクトの支援をしております。
このプロジェクトでは、6人チームでWEBアプリを開発しているため、コラボレーション強化のため本研修を実施しました。

#### 活動2-2:学生向けGitHub研修
学生向けに2時間の研修を6セッション実施しました。

学生は卒業プロジェクトとして、5,6人のチームでアプリを開発します。
改善プロジェクトと同じく、コラボレーション強化のため研修を実施しました。

#### 活動2-X:ESFE向けGitHub研修
Sonsonate県の姉妹校ESFEでも、教員10名を対象にGitHub研修を実施しました。

私は受講者ではなく講師だったのですが、なぜか私も修了証をもらいました。
Yo era instructor, no participante, pero de alguna manera también recibí un diploma.

### 活動3:生産性向上
ミッション達成のために、typing.comとTypingClubという無料サービスを使ってタイピング教室を実施しました。

#### 活動3-1:タイピング教室
コンピュータ技術学科に加えて、マーケティング学科の学生に対しても全○回実施しました。
自由参加にしたため、学生があまり集まらず苦い思い出です。
しかし、個人的に良いスペイン語でのタイピング練習になりました。

## 1年目の成果と課題（専門分野での日本との相違、悩んだこと、困難だったこと含む）
### 1年目の成果
次に1年目の成果を紹介いたします。

Androidアプリ開発授業の教材が完成。
例年、 デモンストレーション中心の授業でしたが、今年は全クラス分の教材を作成しました。
来年度以降も内容をレビュー、更新しつつ、再利用する予定であるため、教員の負担軽減を期待できます。

教員のプロジェクトでGitHubによるコード共有が実現。
GitHub研修の結果、教員のプロジェクトでの、GitHubを用いたコードの共有を開始できました。
ForresterのリサーチによるとGitHub導入効果は○○だそうです。

卒業制作で90%のチームがGitHubを導入した。
導入率50%が目標でした。
GitHub研修の結果、10チーム中9チームが卒業制作でGitHubを利用しました。
  
### 悩んだこと
1年目は、自分に合った教材づくりの方法を見つけるのに苦労しました。
悩み①は、公式の教材の使い勝手が悪かった点です。
活動初期は、教材づくりの手間を省くためにGoogleが作成したAndroid公式サイトの教材を利用していた。
しかし、この教材はあまりにも情報が豊富すぎる上に、自身で編集できないため、使い勝手が悪かった。
さらに、使われている単語も難しく、授業のたびに単語の練習が必要で結果的に時間がかかりました。

そこで解決策として、自身で教材を作ることにしました。


```
自身でMarkdown形式で教材を作成し、GitHubを使って公開した。
情報の粒度が適切になった。ミスなどがあれば、LIVEで修正反映できて非常に便利だった。
また、Markdownは表現力豊かで、ソースコードを目立たせたり、折りたたみセクションを使って練習問題の答えを隠したりできて自分好みの教材を作れた。
```

しかし、新たな悩みが発生しました。
面倒くさい。

解決策として、ChatGPTとGitHubを組み合わせて教材を作りました。
ChatGPTの回答は、太文字や箇条書きなどで見た目が整えられた形式で表示されます。
この見た目の調整はMarkdownという技術です。
GitHubはMarkdownをサポートしているので、ChatGPTの回答をそのまま張り付けるだけで教材を作成できました。
教材作成時間が大幅に短縮された。授業内の演習問題はほとんどChatGPTに作って頂きました。
ChatGPTの回答が間違っていることがあるので、自身で必ず検証していました。

### 課題
トレンド技術の知識がない。
これは、私の課題であり、組織の課題でもあると思っています。
2年に一回、カリキュラムのレビューを実施し、常に更新している。これはとても素晴らしい取り組みです。
しかし、このタジェールは、企業で需要の高いプログラミング言語やフレームワークが何かは調査しきれていない。
冒頭で話した通り、技術は星の数ほどあります。
プログラミング言語はもちろん多種多様で、さらに同じ言語でも型が複数あります。

心配しないでください。
課題は伸びしろです。

## 今後の展望
①企業向けのトレンド技術調査
先ほど言及した課題の対処方として、企業向けのトレンド技術調査を実施します。
ITCHAの生徒のインターンとして受け入れしている企業、卒業生の就職先、業界団体から紹介された企業を対象に実施します。
調査結果を考慮し次年度の研修テーマを考案します。

②ITCHAアプリの再構築
2018にITCHAのアプリがリリースされたのですが、メンテナンス不備でポリシー違反となり、ストアから削除されいています。
今日アプリは利用されている技術が古いため、新しい技術を使ってアプリを再構築します。

# おまけ
要請からは外れますが、寿司の普及活動に注力したので少し紹介させて頂きます。

次の活動を実施しました。
配属先および友人の家でワークショップ○回実施しました。。
Agroの穀物の単元で寿司の特別授業を実施しました。
Youtube出演し、寿司のデモンストレーションを実施しました。
https://www.youtube.com/watch?v=ZKkfnefLNLc
![image](https://github.com/user-attachments/assets/6b39fef0-e572-4ab5-9cdd-288065eaab3b)

技能向上のために、見学をしました。
10月に帰任された料理のボランティアの淺井さんとChalatenango県で寿司を販売しているエルサルバドル人のWiliamさんの見学をしました。

そして、今日は師匠の寿司を用意しました。師匠が独自に開発したTropicalロールです。
こちらプレゼントさせて頂きますので、ぜひお試しください。
なお、材料はこちらの一覧の通りです。念のためご確認ください。

以上です。有難うございました。
